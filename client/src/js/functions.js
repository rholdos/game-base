  ...
  if (id) {
    serverAxios()
      ...
      .then((response) => {
        ...
        serverAxios()
          .get(`/users/${clan.leader}`)
          .then((response) => {
            clan.leader = response.data;
          });
        if (clan.member_2) {
          serverAxios()
            .get(`/users/${clan.member_2}`)
            .then((response) => {
              clan.member_2 = response.data;
            });
        }
        if (clan.member_3) {
          serverAxios()
            .get(`/users/${clan.member_3}`)
            .then((response) => {
              clan.member_3 = response.data;
            });
        }
        if (clan.member_4) {
          serverAxios()
            .get(`/users/${clan.member_4}`)
            .then((response) => {
              clan.member_4 = response.data;
            });
        }
        if (clan.member_5) {
          serverAxios()
            .get(`/users/${clan.member_5}`)
            .then((response) => {
              clan.member_5 = response.data;
            });
        }
  ...
  if (id) {
    serverAxios()
      ...
      .then((response) => {
        ...
        serverAxios()
          .get(`/users/${response.data.leader}`)
          .then((response) => {
            clan_invitation.leader = response.data.username;
          });
        clan_invitation.members = 1;
        if (response.data.member_2) {
          clan_invitation.members++;
        }
        if (response.data.member_3) {
          clan_invitation.members++;
        }
        if (response.data.member_4) {
          clan_invitation.members++;
        }
        if (response.data.member_5) {
          clan_invitation.members++;
        }
